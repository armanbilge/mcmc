...
class UpDownOperator[T, @specialized(Double) R : Field : Trig](val scaleFactor: R, val up: Traversable[Lens[T, R]], val down: Traversable[Lens[T, R]])(implicit val rng: Generator) extends Operator[T, R] {
...
object UpDownOperator {
  ...
  implicit def coercer[T, R : Field : Trig]: OperatorCoercer[T, R, UpDownOperator[T, R]] =
    OperatorCoercer[T, R, UpDownOperator[T, R]](op => Trig[R].log(1 / op.scaleFactor - 1))(x => op => new UpDownOperator[T, R](1 / (Trig[R].exp(x) + 1), op.up, op.down)(Field[R], Trig[R], op.rng))
