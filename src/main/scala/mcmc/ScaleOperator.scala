...
class ScaleOperator[T, @specialized(Double) R : Field : Trig](val scaleFactor: R, val lenses: Traversable[Lens[T, R]])(implicit val rng: Generator) extends Operator[T, R] {
...
object ScaleOperator {
  ...
  implicit def coercer[T, R : Field : Trig]: OperatorCoercer[T, R, ScaleOperator[T, R]] =
    OperatorCoercer[T, R, ScaleOperator[T, R]](op => Trig[R].log(1 / op.scaleFactor - 1))(x => op => new ScaleOperator[T, R](1 / (Trig[R].exp(x) + 1), op.lenses)(Field[R], Trig[R], op.rng))
